...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        h = x                   # x.shape : 640 x 640
        h = self.conv1(h)       # h : 64 x 320 x 320
        h = self.bn1(h)
        h = self.relu1(h)
        h = self.maxpool(h)     # h : 64 x 160 x 160
        h = self.layer1(h)      # h : 256 x 160 x 160
        c2 = h
        h = self.layer2(h)      # h : 512 x 80 x 80
        c3 = h
        h = self.layer3(h)      # h : 1024 x 40 x 40
        c4 = h
        h = self.layer4(h)      # h :  2048 x 20 x 20
        c5 = h

        # Top-down
        p5 = self.toplayer(c5)  # p5 256 x 20 x 20
        p5 = self.toplayer_relu(self.toplayer_bn(p5))
        c4 = self.latlayer1(c4) # c4 256 x 40 x 40 
        c4 = self.latlayer1_relu(self.latlayer1_bn(c4)) # c4 256 x 40 x 40 
        p4 = self._upsample_add(p5, c4) # 把p5 upsamle到c4的水平，并加上c4 ， p4: 256 x 40 x 40
        p4 = self.smooth1(p4)           # 使用一个3*3 的卷积核抽取特征 , p4 :256 x 40 x 40
        p4 = self.smooth1_relu(self.smooth1_bn(p4))

        c3 = self.latlayer2(c3)         # 256 x 80 x 80
        c3 = self.latlayer2_relu(self.latlayer2_bn(c3)) 
        p3 = self._upsample_add(p4, c3) # 256 x 160 x 160 
        p3 = self.smooth2(p3)           
        p3 = self.smooth2_relu(self.smooth2_bn(p3))        

        c2 = self.latlayer3(c2)         # 256 x 160 x 160
        c2 = self.latlayer3_relu(self.latlayer3_bn(c2))
        p2 = self._upsample_add(p3, c2) # 256 x 320 x 320
        p2 = self.smooth3(p2)      
        p2 = self.smooth3_relu(self.smooth3_bn(p2)) #256 x 320 x 320

        # 到这里就完成了p5~p2的处理
        # 随后在下面的这个部分把所有的p层都放大到同一个尺度
        p3 = self._upsample(p3, p2)     # 直接把p3upsample到p2 的尺度 ，p3 256 x 320 x 320 
        p4 = self._upsample(p4, p2)         
        p5 = self._upsample(p5, p2)

        out = torch.cat((p2, p3, p4, p5), 1) # 直接cat并处理
        out = self.conv2(out)
        out = self.relu2(self.bn2(out))
        out = self.conv3(out)
        out = self._upsample(out, x, scale=self.scale) # 再pusample到x的维度
