...
def ohem_single(score, gt_text, training_mask):
    ...
    # selected_mask取1的地方才进行处理，selected_mask取零的地方不进行判断
...
def ohem_batch(scores, gt_texts, training_masks):
    ...
    for i in range(scores.shape[0]):
        # 对这里ohem的处理：
        # 首先是这样的，对于每张图，一般情况下，都会有正例，有负例
        # 然后这里ohem的作用在于，一般一张图上的负例的数量都会比正例的多得多（图片中的负例较多）
        # 则训练的时候，只会取出那些预测为负例概率最低的位置的负例样本作为这张图上的负例（使得正负比例1:3）
...
def dice_loss(input, target, mask):
    input = torch.sigmoid(input)   # input : 16 * 640 * 640

    # 在语义分割中，我们通常会使用 “甩平” -> “计算loss”的方式
    input = input.contiguous().view(input.size()[0], -1)      #input :16 *4096 (甩平，目的是为了使用pytorch提供的loss函数)
    target = target.contiguous().view(target.size()[0], -1)   
    mask = mask.contiguous().view(mask.size()[0], -1)
    
    # 这里会使用mask，将需要计算相关性的点标注出来，对于不需要计算相关性的点，不作处理
    # 可惜的是，这个处理用到的损失函数是一个语义分割的损失函数    
    input = input * mask
    target = target * mask
...
def main(args):
    ...
    data_loader = IC15Loader(is_transform=True, 
                            img_size=args.img_size, kernel_num=kernel_num, min_scale=min_scale
